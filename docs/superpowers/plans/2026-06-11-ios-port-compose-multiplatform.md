...
- [x] **compose-multiplatform plugin + commonMain promotion (Linux-completable part).** Done in 4 verified commits, Android green throughout (99 tests):
  1. Applied `org.jetbrains.compose` to `:shared` AND `:androidApp`; dropped the androidx compose-BOM; switched to `compose.*` DSL accessors (deprecated but functional in 1.9.3, and they auto-version material3 etc. — safer than hand-pinning each independently-versioned org.jetbrains.compose coordinate). `compose.materialIconsExtended` replaces the manual androidx icons dep. ui-test-junit4 → `org.jetbrains.compose.ui:ui-test-junit4:1.9.3`; ui-test-manifest (no JB variant) → `androidx.compose.ui:ui-test-manifest:1.11.2` (the androidx ui version CMP 1.9.3 maps to — see `composeUiAndroid` in the catalog).
  2. Promoted the platform-free **domain + data** layer to `commonMain` (models, DTOs, parsers, Ktor APIs, repo interfaces, DefaultCollectionRepository). Two portability fixes: `StoreCartLinks` `URLEncoder`→Ktor `encodeURLParameter`; `CardName` NFKD-deaccent (Normalizer + `\p{Mn}`, neither Native-safe) extracted to `expect/actual deaccentNfkd()` — **android actual provided, iOS actual is a Phase 3 TODO**.
  3. Relocated all multiplatform UI/DI deps (compose.*, koin core/compose/viewmodel, JetBrains nav, coil3, confettikit) to `commonMain`; swapped androidx lifecycle-compose → **JetBrains `org.jetbrains.androidx.lifecycle` 2.9.6** (same `androidx.lifecycle.*` packages, the version Koin already resolves to). `androidMain` keeps only android-only deps.
  4. Promoted the **bitmap-free UI**: theme, widgets (CardThumbnail/CompactSearchField/FoilSheen/SpeedDialFab/AlphabetRail/AlphabetIndexedColumn), CelebrationOverlay, AddDeckScreen, and the coupling-free ViewModels (DeckList/Import/Pull/ShoppingList). **commonMain = 40 files.**

**PHASE 3 + 4 DONE (2026-06-12) — CI is green and produces a downloadable unsigned `.ipa`.** GitHub Actions `iOS` workflow: `compile` (macos-14) + `ipa` (macos-15) both green; artifact `DeckPuller-unsigned-ipa` (~16 MB) uploads per run. Android build + 99 tests stayed green throughout.

What landed in Phase 3/4:
- Every Android-bound behavior is a `com.deckpuller.platform` expect/actual: `rememberHaptics` (UIKit feedback generators), `rememberDeviceTilt` (CoreMotion), `loadCardBitmap`, `rememberCsvPicker` (UIDocumentPicker), `rememberUrlOpener` (UIApplication.openURL), `nowMillis`/`formatImportDate` (NSDate/NSDateFormatter), `httpClientEngine` (Darwin). Self-update is expect/actual: `UpdateGate`/`SettingsRoute` real on Android, no-op on iOS (`UpdateStatus`/`UpdateInfo` promoted to commonMain). DI split into commonMain `sharedModule` + `androidModule`/`iosModule`. All UI promoted to commonMain (`App()` entry); only genuine platform code + actuals remain in androidMain/iosMain.
- iOS shell: `shared/src/iosMain` (actuals, `MainViewController`, `doInitKoin`), `iosApp/` (XcodeGen `project.yml` + SwiftUI), `.github/workflows/ios.yml`, `docs/IOS_SIDELOAD.md`.

**Version lesson (important):** the Kotlin/Native compiler is strict about klib ABI; the JVM/Android build is not. Ktor 3.5.0, Coil 3.5.0, Koin 4.2.1, ConfettiKit 0.8.0 all ship iOS klibs built with Kotlin 2.3.x, which the project's **Kotlin 2.2.20** Native compiler rejects. Pinned them DOWN to their Kotlin-2.2 lines: **Ktor 3.3.0, Coil 3.3.0, Koin 4.1.0, ConfettiKit 0.7.0** (rather than bump Kotlin/CMP/KSP/Room and risk the validated Android stack). Also: Gradle heap raised to `-Xmx6g` (Native links the Release framework in-process and OOM'd at 2 GB); `Dispatchers.IO` is `internal` on Native (Room query context uses `Dispatchers.Default` on iOS); `String.format`/`Bundle.getLong` aren't multiplatform (replaced with a hand-rolled `usd()` and `androidx.savedstate.read { getLong }`).

**Remaining (need a physical iPhone / optional polish):** Task 4.2 app icon (skipped — build is green without it; add an `Assets.xcassets/AppIcon.appiconset` + `ASSETCATALOG_COMPILER_APPICON_NAME`); Task 4.4 on-device feel test (haptics/gyro/CSV-import/share/Room round-trip) via SideStore — the real verification gate for the iOS actuals; `loadCardBitmap` returns null on iOS so the shatter/cascade flourish is skipped (TODO: Skia decode); CoreMotion tilt sign may need flipping vs Android. Task 4.5 (make `iOS / compile` a required check) left to the user (branch protection).

---

(Historical, pre-Phase-3 snapshot:) The 21 files then in `androidMain` were all genuine platform seams: `CardBitmap` (Coil `Image`→`ImageBitmap`), `PullFeedback` (haptics), `CardImageDialog` (sensors+haptics), `CollectionImporter` (content resolver), `ImagePrefetcher`, `UpdateManager`/`UpdateGate`/`UpdateViewModel` (self-update → iOS no-op), `CollectionScreen`/`DefaultDeckRepository` (`DateFormat`/`System.currentTimeMillis`), `AppModule` (android DI), `AppRoot` (nav host), plus the screens that compose those (`DeckListScreen`, `PullScreen`, `ShoppingListScreen`, `SettingsScreen`, `CollectionViewModel`, `CardRow`, `CardShatterOverlay`, `DeckCompletionCascade`). The `expect` side + android `actual`s can be drafted on Linux; the iOS `actual`s (CoreMotion tilt, CoreHaptics, UIDocumentPicker, share sheet, bitmap decode, clock) need macOS to compile/verify. Then Phase 4 = CI macOS build + SideStore sideload.
